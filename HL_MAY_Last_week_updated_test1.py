                    ...
                    if remain > 0:
                        ...
                        if london_list.index(params[0]) <= 4:
                            ...
                            if (london_exit_df.loc[london_exit_df.CURRENCY == params[0], "BID"].values[0] != 'YES') and \
                                ...
                                print(params[0] + "ASK", rt_price[0])
                                print(params[0] + "BID", rt_price[1])
                                print(params[0] + "MID", rt_price[2])
                                print(params[0] + "TP", tp_buy)
...
def run_london():
    ...
    if currtime < "16:26:00":
        sleeptime = (
                datetime.strptime("16:26:01", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
        print("LONDON cycle is sleeping till scheduled time 16:26", sleeptime)
        time.sleep(sleeptime)
    ...
    if shortsum > 1 and longsum > 1:
        ...
        currtime = datetime.now().strftime('%H:%M:%S')
        if currtime < "16:29:25":
            sleeptime = (
                    datetime.strptime("16:29:26", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
            print("LONDON ENTRIES WILL START AT 16:29:26, SECS LEFT: ", sleeptime)
            time.sleep(sleeptime)
