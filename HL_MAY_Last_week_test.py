...
def run_london():
    ...
    if shortsum > 1 and longsum > 1:
        ...
        currtime = datetime.now().strftime('%H:%M:%S')
        if currtime < "14:29:25":
            sleeptime = (
                    datetime.strptime("14:29:26", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
            print("LONDON ENTRIES WILL START AT 14:29:26, SECS LEFT: ", sleeptime)
            time.sleep(sleeptime)
