...
def transmit_entry(dat,curr_name, qty, contract, ids, round1, signal, tot_time, fav_time, mid_time, tickerId, closetime):
    qty = abs(qty)
            ...
            if price > 0:
                ...
                if confirm[confirm.orderId == id].empty == False:
                    ...
                    if remain > 0:
                        ...
                        exeprice = float(round((exeprice), 4))
                        exetime = datetime.now().strftime("%Y:%m:%d %H:%M:%S")
...
london_log = pd.DataFrame(
        {"CURRENCY": london_list, "ENTRY_SIGNAL": np.nan, "ENTRY_FILLED": np.nan, "ENTRY_TYPE": np.nan, "EXE_TIME": np.nan, "ENTRY_REMAIN": np.nan, "TP_BUY": np.nan, "TP_BUY2": np.nan,
         "BUY_QTY": np.nan, "TP_SELL": np.nan, "TP_SELL2": np.nan, "SELL_QTY": np.nan, "CLOSE": np.nan,"ENTRY_EXE":np.nan})

excel_df=pd.DataFrame(columns=["CURRENCY","TP_BUY","TP_SELL"])
...
ny_df = pd.DataFrame(
    {"CURRENCY": ny_list, "OPEN_POSITION": np.nan, "ENTRY_QTY": np.nan, "ENTRY_SIGNAL": np.nan, "ASK": np.nan,
     "BID": np.nan,
     "ENTRY_EXE": np.nan, "ENTRY_TIME": np.nan,"ENTRY_TYPE":np.nan,
     "LONG_EXIT_QTY": np.nan, "LONG_TP": np.nan, "LONG_EXIT_EXE": np.nan, "LONG_EXIT_TIME": np.nan,
     "SHORT_EXIT_QTY": np.nan, "SHORT_TP": np.nan, "SHORT_EXIT_EXE": np.nan, "SHORT_EXIT_TIME": np.nan,
     "CLOSE": np.nan, "ACT_RETURN": np.nan, "CYCLE_DATE": np.nan})
...
ny_log = pd.DataFrame(
        {"CURRENCY": ny_list, "ENTRY_SIGNAL": np.nan, "ENTRY_FILLED": np.nan, "ENTRY_TYPE": np.nan,
         "EXE_TIME": np.nan,
         "ENTRY_REMAIN": np.nan, "TP_BUY": np.nan, "TP_BUY2": np.nan,
         "BUY_QTY": np.nan, "TP_SELL": np.nan, "TP_SELL2": np.nan, "SELL_QTY": np.nan, "CLOSE": np.nan,"ENTRY_EXE":np.nan})

#####################


def LONDON(dat,params, ids, tickerId, histId, london_df,london_log,excel_df):
    tp_buy = 0
    tp_buy2 = 0
    tp_sell = 0
    tp_sell2 = 0
    ...
    print("%%%%%%%%%%%%%%%%%%%%%%%%")
    print("")
    # with open('C:\TP_VALUE_LOG/london_log.txt', "a") as f:
    #     # f.write("\n")
    ...
    if london_list.index(params[0]) > 5:
        london_log.loc[
            london_log.CURRENCY == params[0], ["TP_BUY", "TP_BUY2", "TP_SELL", "TP_SELL2", "BUY_QTY", "SELL_QTY",
                                               "ENTRY_FILLED", "ENTRY_REMAIN","ENTRY_SIGNAL","ENTRY_TYPE","EXE_TIME","CLOSE","ENTRY_EXE"]] = float(tp_buy), float(tp_buy2), float(
            tp_sell), float(tp_sell2), int(params[7]), int(params[6]), int(entry_result[2]), int(entry_result[3]),entry_result[6],entry_result[5],entry_result[4],float(close_data),entry_result[0]
    elif london_list.index(params[0]) <= 4:
        london_log.loc[
            london_log.CURRENCY == params[0], ["TP_BUY", "TP_BUY2", "TP_SELL", "TP_SELL2", "BUY_QTY", "SELL_QTY",
                                               "ENTRY_FILLED", "ENTRY_REMAIN","ENTRY_SIGNAL","ENTRY_TYPE","EXE_TIME","CLOSE","ENTRY_EXE"]] = float(tp_buy), float(tp_buy2), float(
            tp_sell), float(tp_sell2), int(params[2]), "0", int(entry_result[2]), int(entry_result[3]),entry_result[6],entry_result[5],entry_result[4],float(close_data),entry_result[0]
    elif london_list.index(params[0]) == 5:
        london_log.loc[
            london_log.CURRENCY == params[0], ["TP_BUY", "TP_BUY2", "TP_SELL", "TP_SELL2", "BUY_QTY", "SELL_QTY",
                                               "ENTRY_FILLED", "ENTRY_REMAIN","ENTRY_SIGNAL","ENTRY_TYPE","EXE_TIME",
                                               "CLOSE","ENTRY_EXE"]] = float(tp_buy), float(tp_buy2),\
                                                                       float(tp_sell), float(tp_sell2), "0", int(params[2]), \
                                                                       int(entry_result[2]), int(entry_result[3]),entry_result[6],\
                                                                       entry_result[5],entry_result[4],float(close_data),entry_result[0]

    london_log = london_log[
        ["CURRENCY", "CLOSE", "ENTRY_SIGNAL", "ENTRY_TYPE", "ENTRY_EXE", "EXE_TIME", "ENTRY_FILLED", "ENTRY_REMAIN",
         "TP_BUY",
         "TP_BUY2", "BUY_QTY", "TP_SELL", "TP_SELL2", "SELL_QTY"]]

    print("shape of LDN log", london_log.shape)
    print("LONDON LOG")
    print(london_log)

    excel_df = excel_df.append([{'CURRENCY': params[0], 'TP_BUY': float(tp_buy), 'TP_SELL': float(tp_sell)}],
                               ignore_index=True)
    print("EXCEL DF")
    print(excel_df)
    #london_log.to_html('C:/REPORTS/htmltrial.html', index=False)
    # time.sleep(2)
    # path = "C:\LOG/" + "log_london.xlsx"
    # writer = pd.ExcelWriter(path)
    # london_log.to_excel(writer, 'LONDON')
    # writer.save()
    ...
    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
        ...
        london_df.loc[london_df.CURRENCY == params[0], "LONG_TP"] = tp_buy
        london_df.loc[london_df.CURRENCY == params[0], "SHORT_TP"] = tp_sell
        ...
        path = "C:\REPORTS/" + entry_result[-1] + "_LONDON.xlsx"
        writer = pd.ExcelWriter(path)
        london_df.to_excel(writer, 'LONDON')
        writer.save()
        ...
        while pos != 0:
            ...
            if london_list.index(params[0]) <= 4:
                ...
                if condition_buy == "NOT FULFILLED":
                    ...
                    if flag_buy == False:
                        if rt_price[2] <= tp_buy:
                            print(params[0], " 20% BUY MET", "RT PRICE:", rt_price[2], "TP 20:", tp_buy)
                            flag_buy=True
                    ...
                    if flag_buy==True and rt_price[0]<= tp_buy2:
                        ...
                        signal = "BUY"
                        fetchtime = datetime.now().strftime("%Y%m%d %H:%M:%S")
                        print("CONDITION MET FOR", params[0], signal, "at", fetchtime, "MID PRICE:",rt_price[2], "TRYING TO PLACE AT :",rt_price[0])
                        id = int(ids[1][0])
                ...
                if condition_sell == "NOT FULFILLED":
                    ...
                    if flag_sell==False:
                        if rt_price[2] >= tp_sell:
                            print(params[0], " 20% SELL MET", "RT PRICE:", rt_price[2], "TP 20:", tp_sell)
                            flag_sell=True
                    ...
                    if flag_sell==True and rt_price[1]>= tp_sell2:
                        ...
                        signal = "SELL"
                        fetchtime = datetime.now().strftime("%Y%m%d %H:%M:%S")
                        print("CONDITION MET FOR", params[0], signal, "at", fetchtime, "MID PRICE:", rt_price[2], "TRYING TO PLACE AT TP:",rt_price[1])
                        id = int(ids[0][0])
                ...
                if flag_sell==True and rt_price[1]>= tp_sell2:
                    if condition_sell == "NOT FULFILLED":
                        signal = "SELL"
                        fetchtime = datetime.now().strftime("%Y%m%d %H:%M:%S")
                        print("CONDITION MET FOR", params[0], signal, "at", fetchtime, "MID PRICE:", rt_price[2], "TRYING TO PLACE AT:",rt_price[1])
                        print(signal, condition_sell)
                        id = int(ids[0][0])
                        # print("ID IS,", id)
                        # print(rt_price[1], tp_sell)
                        price = float(round(rt_price[1], round1))  #### CHANGED FROM REAL TIME PRICE TO CALCULATED TP PRICE FOR EXECUTION #### @@ 20% INITIALLY ###
                        if pos_diff < 0:
            ...
            if condition1 == "MET":
                ...
                if confirm[confirm.orderId == id].empty == False:
                    ...
                    if remain > 0:
                        ...
                        # london_df.loc[london_df.CURRENCY == params[0], "OPEN_POSITION"] = list1[1][9]
                        ...
                        # london_df.loc[london_df.CURRENCY == params[0], "ENTRY_EXE"] = list1[1][0]
                        ...
                        # london_df.loc[london_df.CURRENCY == params[0], "CLOSE"] = list1[-1]
                        # london_df.loc[london_df.CURRENCY == params[0], "CYCLE_DATE"] = list1[1][-1]

                        if london_list.index(params[0]) <= 4:
                            london_df.loc[london_df.CURRENCY == params[0], "LONG_EXIT_QTY"] = exit_buy_list[2]
                            london_df.loc[london_df.CURRENCY == params[0], "LONG_TP"] = exit_buy_list[0]
                            london_df.loc[london_df.CURRENCY == params[0], "LONG_EXIT_EXE"] = exit_buy_list[1]
                            london_df.loc[london_df.CURRENCY == params[0], "LONG_EXIT_TIME"] = exit_buy_list[4]
                            ...
                            london_df.loc[london_df.CURRENCY == params[0], "SHORT_EXIT_QTY"] = exit_sell_list[2]
                            london_df.loc[london_df.CURRENCY == params[0], "SHORT_TP"] = exit_sell_list[0]
                            london_df.loc[london_df.CURRENCY == params[0], "SHORT_EXIT_EXE"] = exit_sell_list[1]
                            london_df.loc[london_df.CURRENCY == params[0], "SHORT_EXIT_TIME"] = exit_sell_list[4]
            ...
            # if condition1 == "MET ALREADY":
            #     # print("EXIT TRADE HAS BEEN EXECUTED AND POS IS 0", params[0])
            #     dat = open_pos()
            #     if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
            #             int(dat[(dat["Local Symbol"] == params[0])].iloc[-1]['Position']) == 0):
            #         # print(pos)
    ...
    #
    # london_df.loc[london_df.CURRENCY == params[0], "OPEN_POSITION"] = list1[1][9]
    # london_df.loc[london_df.CURRENCY == params[0], "ENTRY_QTY"] = list1[1][2]
    # london_df.loc[london_df.CURRENCY == params[0], "ENTRY_SIGNAL"] = list1[1][6]
    # london_df.loc[london_df.CURRENCY == params[0], "ASK"] = list1[1][7]
    # london_df.loc[london_df.CURRENCY == params[0], "BID"] = list1[1][8]
    # london_df.loc[london_df.CURRENCY == params[0], "ENTRY_EXE"] = list1[1][0]
    # london_df.loc[london_df.CURRENCY == params[0], "ENTRY_TYPE"] = list1[1][5]
    # london_df.loc[london_df.CURRENCY == params[0], "ENTRY_TIME"] = list1[1][4]
    # london_df.loc[london_df.CURRENCY == params[0], "CLOSE"] = list1[-1]
    # london_df.loc[london_df.CURRENCY == params[0], "CYCLE_DATE"] = list1[1][-1]
    #
    # if london_list.index(params[0]) <= 4:
    #     london_df.loc[london_df.CURRENCY == params[0], "LONG_EXIT_QTY"] = list1[2][2]
    #     london_df.loc[london_df.CURRENCY == params[0], "LONG_TP"] = list1[2][0]
    #     london_df.loc[london_df.CURRENCY == params[0], "LONG_EXIT_EXE"] = list1[2][1]
    #     london_df.loc[london_df.CURRENCY == params[0], "LONG_EXIT_TIME"] = list1[2][4]
    #     # if condition_buy == 'FULFILLED':
    #     #     london_df.loc[london_df.CURRENCY == params[0], "ACT_RETURN"] = round(
    #     #         (exit_buy_list[1] / entry_result[0] - 1) * (-1) * 100, 2)
    #
    # elif london_list.index(params[0]) == 5:
    #     london_df.loc[london_df.CURRENCY == params[0], "SHORT_EXIT_QTY"] = list1[3][2]
    #     london_df.loc[london_df.CURRENCY == params[0], "SHORT_TP"] = list1[3][0]
    #     london_df.loc[london_df.CURRENCY == params[0], "SHORT_EXIT_EXE"] = list1[3][1]
    #     london_df.loc[london_df.CURRENCY == params[0], "SHORT_EXIT_TIME"] = list1[3][4]
    #     # if condition_sell == 'FULFILLED':
    #     #     london_df.loc[london_df.CURRENCY == params[0], "ACT_RETURN"] = round(
    #     #         (exit_sell_list[1] / entry_result[0] - 1) * 100, 2)
    #
    ...
    #     if params[0] == 'AUD.NZD':
    #        table = db.Aud_Nzd
    #     elif params[0] == "CHF.NOK":
    #         table = db.Chf_Nok
    #     elif params[0] == "CHF.SEK":
    #         table = db.Chf_Sek
    #     elif params[0] == "NZD.CHF":
    #         table = db.Nzd_Chf
    #     elif params[0] == "GBP.SEK":
    #         table = db.Gbp_Sek
    #     elif params[0] == "EUR.NZD":
    #         table = db.Eur_Nzd
    #     elif params[0] == "EUR.NOK":
    #         table = db.Eur_Nok
    #     elif params[0] == "EUR.SEK":
    #         table = db.Eur_Sek
    #     elif params[0] == "GBP.NOK":
    #         table = db.Gbp_Nok
    #     elif params[0] == "NOK.SEK":
    #         table = db.Nok_Sek
    #     else:
    #         table = ""
    #
...
def NY(dat,params, ids, tickerId, histId, ny_df,ny_log):
    tp_buy = 0
    tp_buy2 = 0
    tp_sell = 0
    tp_sell2 = 0
    ...
    if ny_list.index(params[0]) >= 7:
        ny_log.loc[
            ny_log.CURRENCY == params[0], ["TP_BUY", "TP_BUY2", "TP_SELL", "TP_SELL2", "BUY_QTY", "SELL_QTY",
                                           "ENTRY_FILLED", "ENTRY_REMAIN", "ENTRY_SIGNAL", "ENTRY_TYPE", "EXE_TIME",
                                           "CLOSE","ENTRY_EXE"]] = float(tp_buy), float(tp_buy2), float(
        tp_sell), float(tp_sell2), int(params[7]), int(params[6]), int(entry_result[2]), int(entry_result[3]), \
                                                       entry_result[6], entry_result[5], entry_result[4], float(close_data),entry_result[0]
        ...
        ny_log.loc[
            ny_log.CURRENCY == params[0], ["TP_BUY", "TP_BUY2", "TP_SELL", "TP_SELL2", "BUY_QTY", "SELL_QTY",
                                           "ENTRY_FILLED", "ENTRY_REMAIN", "ENTRY_SIGNAL", "ENTRY_TYPE", "EXE_TIME",
                                           "CLOSE","ENTRY_EXE"]] = float(tp_buy), float(tp_buy2), float(
        tp_sell), float(tp_sell2), "0", int(params[2]), int(entry_result[2]), int(entry_result[3]), entry_result[6], \
                                                       entry_result[5], entry_result[4], float(close_data),entry_result[0]
        ...
        ny_log.loc[
            ny_log.CURRENCY == params[0],  ["TP_BUY", "TP_BUY2", "TP_SELL", "TP_SELL2", "BUY_QTY", "SELL_QTY",
                                           "ENTRY_FILLED", "ENTRY_REMAIN", "ENTRY_SIGNAL", "ENTRY_TYPE", "EXE_TIME",
                                           "CLOSE","ENTRY_EXE"]] = float(tp_buy), float(tp_buy2), float(
        tp_sell), float(tp_sell2),int(params[2]),"0", int(entry_result[2]), int(entry_result[3]), entry_result[6], \
                                                       entry_result[5], entry_result[4], float(close_data),entry_result[0]
    ...
    print("shape of NY log",ny_log.shape)

    #ny_log.to_html('C:/REPORTS/htmltrialny.html', index=False)
    time.sleep(random.uniform(0.001,0.200))
    path = "C:\LOG/" + "log_ny.xlsx"
    writer = pd.ExcelWriter(path)
    ny_log.to_excel(writer, 'NY')
    writer.save()
                ...
                if confirm[confirm.orderId == id].empty == False:
                    ...
                    if remain > 0:
                        ...
                        if ny_list.index(params[0]) <= 3:
                            ...
                            ny_df.loc[ny_df.CURRENCY == params[0], "LONG_EXIT_EXE"] = exit_buy_list[1]
                            ny_df.loc[ny_df.CURRENCY == params[0], "LONG_EXIT_TIME"] = exit_buy_list[4]
                            # if condition_buy == 'FULFILLED':
                            ...
                            ny_df.loc[ny_df.CURRENCY == params[0], "SHORT_EXIT_EXE"] = exit_sell_list[1]
                            ny_df.loc[ny_df.CURRENCY == params[0], "SHORT_EXIT_TIME"] = exit_sell_list[4]
                            # if condition_sell == 'FULFILLED':
                            #     ny_df.loc[ny_df.CURRENCY == params[0], "ACT_RETURN"] = round(
                            #         (exit_sell_list[1] / entry_result[0] - 1) * 100, 2)
...
def run_london():
    ...
    london_log = pd.DataFrame(
        {"CURRENCY": london_list, "ENTRY_SIGNAL": np.nan, "ENTRY_FILLED": np.nan, "ENTRY_TYPE": np.nan,'ENTRY_EXE':np.nan,
         "EXE_TIME": np.nan,"ENTRY_REMAIN": np.nan, "TP_BUY": np.nan, "TP_BUY2": np.nan,
         "BUY_QTY": np.nan, "TP_SELL": np.nan, "TP_SELL2": np.nan, "SELL_QTY": np.nan, "CLOSE": np.nan})
    # london_log = london_log[
    #     ["CURRENCY","CLOSE","ENTRY_SIGNAL","ENTRY_TYPE","ENTRY_EXE","EXE_TIME","ENTRY_FILLED", "ENTRY_REMAIN", "TP_BUY",
    #      "TP_BUY2", "BUY_QTY", "TP_SELL", "TP_SELL2", "SELL_QTY"]]
    ...
    if shortsum > 1 and longsum > 1:
        ...
        # currtime = datetime.now().strftime('%H:%M:%S')
        # if currtime < "12:59:20":
        #     sleeptime = (
        #             datetime.strptime("12:59:21", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
        #     print("LONDON ENTRIES WILL START AT 12:59, SECS LEFT: ", sleeptime)
        #     time.sleep(10)
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,AUDNZD_params, ids, 100, 200, london_df,london_log,excel_df)).start()
        ################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,CHFNOK_params, ids1, 300, 400, london_df,london_log,excel_df)).start()
        ###############################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,CHFSEK_params, ids2, 500, 600, london_df,london_log,excel_df)).start()
        ##########################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'NZD.CHF', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,NZDCHF_params, ids3, 700, 800, london_df,london_log,excel_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,GBPSEK_params, ids4, 900, 1000, london_df,london_log,excel_df)).start()
        ###############################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,EURNZD_params, ids5, 1100, 1200, london_df,london_log,excel_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,EURNOK_params, ids6, 1200, 1300, london_df,london_log,excel_df)).start()
        ##############################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,EURSEK_params, ids7, 1400, 1500, london_df,london_log,excel_df)).start()
        ##################################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,GBPNOK_params, ids8, 1600, 1700, london_df,london_log,excel_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'NOK.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,NOKSEK_params, ids9, 1800, 1900, london_df,london_log,excel_df)).start()
        ########################
    return
...
def run_ny():
    ...
    #     sleeptime = (datetime.strptime("14:56:01", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
    ...
    if shortsum > 1 and longsum > 1:
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.SGD', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag in ('yes', 'YES')):
            Thread(target=NY, args=(dat,AUDSGD_params, audsgdids, 2000, 2100, ny_df,ny_log)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.PLN', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag1 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,CHFPLN_params, chfplnids, 2200, 2300, ny_df,ny_log)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.CZK', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag2 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,GBPCZK_params, gbpczkids, 2400, 2500, ny_df,ny_log)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.PLN', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag3 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,GBPPLN_params, gbpplnids, 2600, 2700, ny_df,ny_log)).start()
        ##################################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.ZAR', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag4 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,AUDZAR_params, audzarids, 2800, 2900, ny_df,ny_log)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'DKK.NOK', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag5 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,DKKNOK_params, dkknokids, 3000, 3100, ny_df,ny_log)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.HUF', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag6 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,EURHUF_params, eurhufids, 3200, 3300, ny_df,ny_log)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.CZK', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag7 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,EURCZK_params, eurczkids, 3400, 3500, ny_df,ny_log)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.MXN', 'TP1'].iloc[-1] <= 0.25) and \
                ...
                switchflag8 in ('yes', 'YES')):
            Thread(target=NY, args=(dat,GBPMXN_params, gbpmxnids, 3600, 3700, ny_df,ny_log)).start()
        ###############################################################
...
def backup():
    ...
    time.sleep(120)
    Thread(target=writeexcelfun()).start()
...
# df2=pd.DataFrame()
# def MKT_DATA():
#     Thread(target=ACC_PORTFOLIO()).start()
#     return
...
# sched.start()
#
# job1 = sched.add_job(main, 'cron', day_of_week='0,1,2,3,4', hour=23, minute=26, misfire_grace_time=100)
# job2= sched.add_job(MKT_DATA, 'cron', day_of_week='0,1,2,3,4', hour=23, minute=20,misfire_grace_time=100)

# import schedule

# schedule.every().monday.at("23:26").do(main)
# schedule.every().tuesday.at("23:26").do(main)
# schedule.every().wednesday.at("23:26").do(main)
# schedule.every().thursday.at("23:26").do(main)
# schedule.every().friday.at("23:26").do(main)
# while True:
#     schedule.run_pending()
#     time.sleep(1)


# def exchange_rate(symbol,id):
#     convprice1=0
#     if symbol in commonwealth_curr:
#         contract_Details1 = create.create_contract(symbol, "CASH", "IDEALPRO", "USD", '', '', '', '')
#         data1 = ask_bid(contract_Details1, id)
#         convprice1 = float(data1[0])
#     else:
#         contract_Details1 = create.create_contract("USD", "CASH", "IDEALPRO",symbol, '', '', '', '')
#         data1 = ask_bid(contract_Details1, id)
#         convprice1 = 1/float(data1[0])
#     return convprice1
#
#
# ############# READING DATA FROM ACC AND PORTFOLIO ###########################
# df_port=pd.read_csv("C:/database\FX_DAILY/FOREX_PORTFOLIO.csv")
# df_port.rename(columns={'Trading Class': 'Trading_Class', 'Realised PnL': 'Realised_PnL',
#                                 'Unrealised PnL':'Unrealised_PnL'}, inplace=True)
#
#
# aud_exch_rate=exchange_rate("AUD",100)
# eur_exch_rate=exchange_rate("EUR",100)
# nok_exch_rate=exchange_rate("NOK",100)
#
#
# audnzd_usd=int(round(((df_port.loc[df_port.Trading_Class=="AUD.NZD","Realised_PnL"].iloc[-1])/(float(round((df_port.loc[df_port.Trading_Class=="AUD.NZD","Market Price"].iloc[-1]),4))))*aud_exch_rate))
#
# eurnzd_usd=int(round(((df_port.loc[df_port.Trading_Class=="EUR.NZD","Realised_PnL"].iloc[-1])/(float(round((df_port.loc[df_port.Trading_Class=="EUR.NZD","Market Price"].iloc[-1]),4))))*eur_exch_rate))
#
# noksek_usd=int(round(((df_port.loc[df_port.Trading_Class=="NOK.SEK","Realised_PnL"].iloc[-1])/(float(round((df_port.loc[df_port.Trading_Class=="NOK.SEK","Market Price"].iloc[-1]),4))))*nok_exch_rate))
#
# print(audnzd_usd,eurnzd_usd,nosek_usd)
