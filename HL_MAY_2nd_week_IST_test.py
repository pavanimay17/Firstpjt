...
def transmit_entry(dat,curr_name, qty, contract, ids, round1, signal, tot_time, fav_time, mid_time, tickerId, closetime):
    ...
    if signal == "null":
        ...
        for i in range(tot_time):
            ...
            if week_day != 5 and "NZD" in curr_name:
                if "00:29:50" <= currtime[-8:] <= "00:45:05":
                    sleeptime = (datetime.strptime("00:45:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],
                                                                                               '%H:%M:%S')).total_seconds()
                    print(curr_name + " sleeping till mkt opens at 00:45")
                ...
                if "02:29:50" <= currtime[-8:] <= "02:45:05":
                    sleeptime = (datetime.strptime("02:45:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],
                                                                                               '%H:%M:%S')).total_seconds()
                    print(curr_name + " sleeping till mkt opens at 02:45")
                    time.sleep(sleeptime)  # sleeptime
...
def LONDON(dat,params, ids, tickerId, histId, london_df):
    tp_buy = 0
    tp_buy2 = 0
    tp_sell = 0
    tp_sell2 = 0
...
def NY(dat,params, ids, tickerId, histId, ny_df):
    ...
    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
        ...
        if (week_day == 4):
            ...
            num = int(df_gtd.loc[df_gtd.MKT == 'NY', "SATURDAY"].iloc[-1])
            exp_date1 = datetime.now() + timedelta(days=num)  ##changed for christmas&new year holiday
            exp_date1 = exp_date1.strftime('%Y%m%d') + " " + exp_time
...
def run_london():
    ...
    # if currtime < "22:26:00":
    #     sleeptime = (
    #             datetime.strptime("22:26:01", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
    #     print("LONDON cycle is sleeping till scheduled time 22:26", sleeptime)
    #     time.sleep(sleeptime)
    ...
    # log_df = pd.DataFrame(
    #     {"CURRENCY": london_list, "ENTRY_FILLED": np.nan, "ENTRY_REMAIN": np.nan, "TP_BUY": np.nan, "TP_BUY2": np.nan,
    ...
    #
    # log_df.to_csv("C:/LOG/log_london.csv")
    ####
    df_common_inputs = pd.read_excel('C:/database/FX_DAILY/FOREX_settings.xlsx', 'common_inputs')
    df_london = pd.read_excel('C:/database/FX_DAILY/FOREX_settings.xlsx', 'LONDON_TRADES')
    df_o_val = pd.read_excel('C:/database/FX_DAILY/FOREX_settings.xlsx', 'LONDON_ENTRY')
    ...
    for i in range(len(london_list)):
        ...
        if week_day != 5 and "NZD" in london_list[i]:
            if "00:29:55" <= currtime[-8:] <= "00:45:05":
                sleeptime = (datetime.strptime("00:45:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],
                                                                                           '%H:%M:%S')).total_seconds()
                print(london_list[i] + " sleeping till mkt opens at 00:45")
                time.sleep(sleeptime)  # sleeptime
            ...
            if "02:29:50" <= currtime[-8:] <= "02:45:05":
                sleeptime = (datetime.strptime("02:45:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],
                                                                                           '%H:%M:%S')).total_seconds()
                print(london_list[i] + " sleeping till mkt opens at 02:45")
                time.sleep(sleeptime)  # sleeptime
    ...
    if shortsum > 1 and longsum > 1:
        ...
        # currtime = datetime.now().strftime('%H:%M:%S')
        # if currtime < "22:29:20":
        #     sleeptime = (
        #             datetime.strptime("22:29:21", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
        #     print("LONDON ENTRIES WILL START AT 22:29, SECS LEFT: ", sleeptime)
        #     time.sleep(sleeptime)
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,AUDNZD_params, ids, 100, 200, london_df)).start()
        ################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat,CHFNOK_params, ids1, 300, 400, london_df)).start()
        ###############################################
...
def run_ny():
    ...
    # currtime = datetime.now().strftime('%H:%M:%S')
    # if currtime < "00:26:00":
    #     sleeptime = (
    #                 datetime.strptime("00:26:01", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
    #     print("NY cycle is sleeping till scheduled time 00:26", sleeptime)
    #     time.sleep(sleeptime)
    ...
    for i in range(len(ny_list)):
        ...
        if week_day != 5 and "NZD" in ny_list[i]:
            ...
            if "02:29:50" <= currtime[-8:] <= "02:45:05":
                sleeptime = (datetime.strptime("02:45:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],
                                                                                           '%H:%M:%S')).total_seconds()
                print(ny_list[i] + " sleeping till mkt opens at 02:45")
                time.sleep(sleeptime)  # sleeptime
    ...
    if shortsum > 1 and longsum > 1:
        ...
        # currtime = datetime.now().strftime('%H:%M:%S')
        # if currtime < "00:29:01":
        #     sleeptime = (datetime.strptime("00:29:01", '%H:%M:%S') - datetime.strptime(currtime,
        #                                                                                '%H:%M:%S')).total_seconds()
        #     print("NY ENTRIES WILL START AT 00:29, SECS LEFT :", sleeptime)
        #     time.sleep(sleeptime)
...
if df_schedule.loc[df_schedule.DAY == "SATURDAY", "PERMISSION_NY"].iloc[-1] == "YES" or \
    ...
    schedule.every().saturday.at(runtime).do(run_ny)
