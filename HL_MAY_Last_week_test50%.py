...
def LONDON(dat, params, ids, tickerId, histId, london_df):
    ...
    if london_list.index(params[0]) <= 4:
        ...
        if entry_result[0] > 0:
            tp_buy = round(float((entry_result[0] - (0.5 * LC_MA * entry_result[0]))), round_tp)
        ...
        if entry_result[0] > 0:
            tp_sell = round(float((entry_result[0] + (0.5 * HC_MA * entry_result[0]))), round_tp)
        ...
        while pos != 0:
            ...
            if close_data == 0 and currtime[-8:] >= (closetime + timedelta(seconds=10)).strftime("%H:%M:%S"):
                ...
                while data_size < 2:
                    ...
                    asset_hist_data, tid = historical(time_ldn, contract, "5 D", histId)
                ...
                if london_list.index(params[0]) <= 4:  ##########short model
                    ...
                    price=round(float((close_data - (0.5 * LC_MA * close_data))), round_tp)

                    signal, id, qty = "BUY", int(ids[1][0]), abs(pos)
                    order_info = create.create_order(acc, "LMT", int(qty), signal, float(price), True, 'GTD',
                                                     exp_date1)
                    tws.placeOrder(id, contract, order_info)
                    exitid.append(id)
                    ...
                    price= round(float((close_data + (0.5 * HC_MA * close_data))), round_tp)

                    signal, id, qty = "SELL", int(ids[0][0]), abs(pos)
                    order_info = create.create_order(acc, "LMT", int(qty), signal, float(price), True, 'GTD',
                                                     exp_date1)
                    tws.placeOrder(id, contract, order_info)
                    exitid.append(id)
...
def run_london():
    ...
    if shortsum > 1 and longsum > 1:
        ...
        currtime = datetime.now().strftime('%H:%M:%S')
        if currtime < "14:12:25":
            sleeptime = (
                    datetime.strptime("13:59:26", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
            print("LONDON ENTRIES WILL START AT 13:59:26, SECS LEFT: ", sleeptime)
            time.sleep(sleeptime)
