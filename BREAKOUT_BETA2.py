...
BO_df = pd.DataFrame(
    {"CYCLE_DATE": np.nan, "CURRENCY": BO_list, "OPEN_POSITION": np.nan, "CLOSE": np.nan, "ASK": np.nan, "BID": np.nan,
     "ENTRY_QTY": np.nan, "ENTRY_SIGNAL": np.nan, "ENTRY TP": np.nan, "ENTRY_EXE": np.nan, "ENTRY_TYPE": np.nan,
     "ENTRY_TIME": np.nan, "EXIT_QTY": np.nan, "EXIT_SIGNAL": np.nan, "EXIT_TP": np.nan, "EXIT_EXE": np.nan,
     "EXIT_TIME": np.nan})
...
def LONDON(contract, dat, params, ids, tickerId, histId, BO_df, signal, exit_signal):
    ...
    exit_thresh1 = float(df_common_inputs["bo_exit_tp"].iloc[-1])
    exit_thresh2 = float(df_common_inputs["bo_exit_tp"].iloc[-1])
    lag_val = 1
    MA_val = 20
    ...
    dat['sell_exit'].loc[myindex1] = dat['open'].loc[myindex1] - (
            dat['open'].loc[myindex1] * dat['exit_HC_Spread'].loc[myindex1])

    myindex1 = dat.loc[(dat['sell_exit'] == 0)].index

    dat['sell_exit'].loc[myindex1] = dat['temp'].loc[myindex1] - (
            dat['temp'].loc[myindex1] * dat['exit_HC_Spread'].loc[myindex1])
    dat = dat.dropna()

    # short_entry = (round(dat["short_entry"].iloc[-1], 4))
    sell_exit = (round(dat["sell_exit"].iloc[-1], 4))

    #print("BUY ENTRY:", buy_entry,"SELL EXIT:", sell_exit)

    ################################log data################################################################################
    log_data = [params, buy_entry, sell_exit]
    ...
    while pos != 0:
        ...
        if exit_signal == "SELL":
            ...
            while rt_price[0] == -1 or rt_price[1] == -1:
                ...
                tickerId = random.randint(5000, 8000)
                rt_price = ask_bid(contract, tickerId)
            if rt_price[2] >= sell_exit:
                #print("BID:", rt_price[1], "SELL_EXIT:", sell_exit)
                actual_tp_sell = sell_exit
                fetchtime = datetime.now().strftime("%Y%m%d %H:%M:%S")
    ...
    # BO_df.loc[BO_df.CURRENCY == params[0], "OPEN_POSITION"] = list1[1][9]
    BO_df.loc[BO_df.CURRENCY == params[0], "CYCLE_DATE"] = cycledate
    BO_df.loc[BO_df.CURRENCY == params[0], "CLOSE"] = close_data
    ...
    BO_df.loc[BO_df.CURRENCY == params[0], "ENTRY_SIGNAL"] = params[1]
    BO_df.loc[BO_df.CURRENCY == params[0], "ENTRY_TP"] = entry_buy_list[0]
    ...
    BO_df.loc[BO_df.CURRENCY == params[0], "EXIT_SIGNAL"] = exit_sell_list[3]
    BO_df.loc[BO_df.CURRENCY == params[0], "EXIT_TP"] = exit_sell_list[0]
    BO_df.loc[BO_df.CURRENCY == params[0], "EXIT_EXE"] = exit_sell_list[1]
    BO_df.loc[BO_df.CURRENCY == params[0], "EXIT_TIME"] = exit_sell_list[-1]

    BO_df = BO_df[["CYCLE_DATE", "CURRENCY", "OPEN_POSITION", "ASK", "BID", "CLOSE",
                           "ENTRY_SIGNAL", "ENTRY_QTY", "ENTRY_TYPE", "ENTRY_EXE", "ENTRY_TIME",
                           "EXIT_SIGNAL","EXIT_QTY","EXIT_TP","EXIT_EXE","EXIT_TIME"]]
...
def run_london():
            ...
            if (i <= 14):
                ...
                if BO_list[i] == "CL":
                    ...
                    contract_Details1 = create.create_contract(symbol="CL", secType="FUT",
                                                               ...
                                                               expiry="20180522", multiplier=cl_mult,
                                                               tradingClass=None)
                    data1 = ask_bid(contract_Details1, id)
                    ...
                    CL_qty = max(
                        (int(math.ceil(
                            ((((BO_cap * float(df_BO['CL'].iloc[0])) / (data1[1] * int(cl_mult))) / 10000) * 10000))),
                         1))
    ...
    time.sleep(2)
    EURHUF_params = long_params[10]
    contract_eurhuf = create.create_contract(EURHUF_params[0][0:3], "CASH", "IDEALPRO", EURHUF_params[0][4:7], '', '', '', '')
    ids10 = orderidfun(acc, contract_eurhuf, EURHUF_params[1], id_list, 2)
    id_list = ids10
    ...
    contract_usdncnh = create.create_contract(USDCNH_params[0][0:3], "CASH", "IDEALPRO", USDCNH_params[0][4:7], '', '', '', '')
    ids15 = orderidfun(acc, contract_usdncnh, USDCNH_params[1], id_list, 2)
    id_list = ids15
    ...
    CL_params = long_params[17]
    contract_cl = create.create_contract('CL', "FUT", "NYMEX", 'USD', '', '', '20180522', cl_mult, '')
    ids17 = orderidfun(acc, contract_cl, CL_params[1], id_list, 2)
    id_list = ids17
    ...
    #        args=(
    #        contract_eurczk, dat, EURCZK_params, ids, 100, 200, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #            contract_sgdcnh, dat, SGDCNH_params, ids1, 300, 400, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_eurhkd, dat, EURHKD_params, ids2, 500, 600, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_audcnh, dat, AUDCNH_params, ids3, 700, 800, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_eurusd, dat, EURUSD_params, ids4, 900, 1000, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_chfpln, dat, CHFPLN_params, ids5, 1100, 1200, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_chfcnh, dat, CHFCNH_params, ids6, 1300, 1400, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_audcad, dat, AUDCAD_params, ids7, 1500, 1600, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_noksek, dat, NOKSEK_params, ids8, 1700, 1800, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_audsgd, dat, AUDSGD_params, ids9, 1900, 2000, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_eurhuf, dat, EURHUF_params, ids10, 2100, 2200, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_eurcnh, dat, EURCNH_params, ids11, 2300, 2400, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_audhkd, dat, AUDHKD_params, ids12, 2500, 2600, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_nzdusd, dat, NZDUSD_params, ids13, 2700, 2800, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_audusd, dat, AUDUSD_params, ids14, 2900, 3000, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_usdcnh, dat, USDCNH_params, ids15, 3100, 3200, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(
    #        contract_usdils, dat, USDILS_params, ids16, 3300, 3400, BO_df, long_params[0][1], long_params[0][2])).start()
    # Thread(target=LONDON,
    #        args=(contract_cl, dat, CL_params, ids17, 3500, 3600, BO_df, long_params[0][1], long_params[0][2])).start()
