...
def historicalclose(fortime, con, totaldays, Id1):
    date = datetime.now().strftime("%Y%m%d") + fortime
    # date = datetime.now().strftime("%Y%m%d") + fortime
    df = pd.DataFrame()
    while df.empty:
...
def LONDON(dat, params, ids, tickerId, histId,london_exit_df):
    if params[0] == "AUD.NZD":
        global AUDNZD_report
        AUDNZD_report = ["null"] * 30
        temp = AUDNZD_report

    if params[0] == "CHF.NOK":
        global CHFNOK_report
        CHFNOK_report = ["null"] * 30
        temp = CHFNOK_report

    if params[0] == "CHF.SEK":
        global CHFSEK_report
        CHFSEK_report = ["null"] * 30
        temp = CHFSEK_report

    if params[0] == "NZD.CHF":
        global NZDCHF_report
        NZDCHF_report = ["null"] * 30
        temp = NZDCHF_report

    if params[0] == "GBP.SEK":
        global GBPSEK_report
        GBPSEK_report = ["null"] * 30
        temp = GBPSEK_report

    if params[0] == "EUR.NZD":
        global EURNZD_report
        EURNZD_report = ["null"] * 30
        temp = EURNZD_report

    if params[0] == "EUR.NOK":
        global EURNOK_report
        EURNOK_report = ["null"] * 30
        temp = EURNOK_report

    if params[0] == "EUR.SEK":
        global EURSEK_report
        EURSEK_report = ["null"] * 30
        temp = EURSEK_report

    if params[0] == "GBP.NOK":
        global GBPNOK_report
        GBPNOK_report = ["null"] * 30
        temp = GBPNOK_report

    if params[0] == "NOK.SEK":
        global NOKSEK_report
        NOKSEK_report = ["null"] * 30
        temp = NOKSEK_report
    ...
    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
        ...
        while pos != 0:
            ...
            if close_data == 0 and currtime[-8:] >= (closetime + timedelta(seconds=10)).strftime("%H:%M:%S"):
                lookbacktime = (closetime + timedelta(seconds=11)).strftime(" %H:%M:%S")
                asset_hist_data, tid = historicalclose(lookbacktime, contract, '100 S', histId)
                df1 = pd.DataFrame()
                #search_t = time_ldn[1:]
                search_t = "12:59:59"     #########this row matching with theory
                while df1.empty:
                    df1 = asset_hist_data[asset_hist_data['date'].str.contains(search_t) == True]
                    search_t = (datetime.strptime(search_t, '%H:%M:%S') + timedelta(seconds=1)).strftime('%H:%M:%S')
                ...
                close_data = round(float(df1['close'].iloc[-1]), 5)
                ...
                print("%%%%%%%%%%%%%%%%%%%%%%%%")
                print("HISTORICAL DATA FOR",params[0])
                print(asset_hist_data.tail(15))
                temp[1] = entry_result[-1]
                temp[2] = entry_result[9]
                temp[3] = entry_result[7]
                temp[4] = entry_result[8]
                temp[5] = close_data
                temp[6] = entry_result[6]
                temp[7] = entry_result[2]
                temp[8] = entry_result[5]
                temp[9] = entry_result[0]
                temp[10] = entry_result[4]
                temp[11] = tp_buy
                temp[12] = tp_buy2
                temp[16] = tp_sell
                temp[17] = tp_sell2
                temp[21] = entry_base_buy_tp
                temp[22] = entry_base_sell_tp
                if london_list.index(params[0]) <= 4:
                    temp[13]=int(params[2])
                elif london_list.index(params[0]) == 5:
                    temp[18]=int(params[2])
                elif london_list.index(params[0]) > 5:
                    temp[13]=int(params[7])
                    temp[18]=int(params[6])
                ...
                if condition_buy == "NOT FULFILLED":
                    ...
                    if flag_buy == False:
                        if rt_price[2] <= tp_buy:
                            ...
                            london_exit_df.loc[(london_exit_df.CURRENCY == params[0]) & (
                                        london_exit_df.SIGNAL == "BUY"), "MID"] = 'YES'
                            temp[23] = 'YES'
                            flag_buy = True
                    ...
                    if flag_buy == True:
                        if rt_price[0] <= tp_buy2:
                            ...
                            london_exit_df.loc[(london_exit_df.CURRENCY == params[0]) & (
                                        london_exit_df.SIGNAL == "BUY"), "ASK"] = 'YES'
                            askflag, bidflag = "YES", "NO"
                            id = int(ids[1][0])
                            # print("ID IS,", id)
                            price = float(round(rt_price[0], round1))
                            ...
                            london_exit_df.loc[(london_exit_df.CURRENCY == params[0]) & (
                                        london_exit_df.SIGNAL == "BUY"), "BID"] = 'YES'
                ...
                if confirm[confirm.orderId == Id].empty == False:
                    ...
                    if remain > 0:
                        ...
                        if Signal == "BUY":
                            ...
                            # print(Signal, condition_buy)
                            if exetime_buy == 'null':
                                exetime_buy = datetime.now().strftime("%Y:%m:%d %H:%M:%S")
                            temp[14] = exit_exeprice
                            temp[15] = exetime_buy
                            if temp[24] == 'null':
                                if askflag == 'YES':
                                    temp[24] = 'ASK'
                                    ...
                                    temp[24] = 'BID'
                                elif askflag == 'NO' and bidflag == 'NO':
                                    temp[24] = 'TP'
                            ...
                            if exetime_sell == 'null':
                                exetime_sell = datetime.now().strftime("%Y:%m:%d %H:%M:%S")
                            temp[19] = exit_exeprice
                            temp[20] = exetime_sell
...
def ldn_report():
    ##### empty the df in every run so new set of data is passed

    for i in range(500):
        time.sleep(600)
        df = pd.DataFrame(
            columns=["CYCLE_DATE","CURRENCY","OPEN_POSITION", "ASK", "BID", "CLOSE", "ENTRY_SIGNAL",
                      "ENTRY_QTY","ENTRY_TYPE", "ENTRY_EXE", "ENTRY_TIME","BUY_TP1", "BUY_TP2", "BUY_EXIT_QTY",
                      "BUY_EXIT_EXE", "BUY_EXIT_TIME","SELL_TP1", "SELL_TP2", "SELL_EXIT_QTY",
                      "SELL_EXIT_EXE", "SELL_EXIT_TIME","ENTRY_BASED_TP_BUY", "ENTRY_BASED_TP_SELL", "BUY_MID",
                      "BUY_FILL_TYPE","SELL_MID", "SELL_FILL_TYPE", "HIGH", "LOW","EOD_CLOSE"])

        ##### make a list of all curr report list variable name
        curr_reports = [AUDNZD_report, CHFNOK_report, CHFSEK_report, NZDCHF_report, GBPSEK_report, EURNZD_report,
                        EURNOK_report, EURSEK_report, GBPNOK_report, NOKSEK_report]

        for i in curr_reports:
            df = df.append(pd.Series(i,
                                     ["CURRENCY", "CYCLE_DATE", "OPEN_POSITION", "ASK", "BID", "CLOSE", "ENTRY_SIGNAL",
                                      "ENTRY_QTY", "ENTRY_TYPE", "ENTRY_EXE", "ENTRY_TIME", "BUY_TP1", "BUY_TP2",
                                      "BUY_EXIT_QTY",
                                      "BUY_EXIT_EXE", "BUY_EXIT_TIME", "SELL_TP1", "SELL_TP2", "SELL_EXIT_QTY",
                                      "SELL_EXIT_EXE", "SELL_EXIT_TIME", "ENTRY_BASED_TP_BUY", "ENTRY_BASED_TP_SELL",
                                      "BUY_MID",
                                      "BUY_FILL_TYPE", "SELL_MID", "SELL_FILL_TYPE", "HIGH", "LOW", "EOD_CLOSE"]
                                     ),
                           ignore_index=True)

        path = "C:\REPORTS/" + curr_reports[0][1] + "_LONDON.csv"
        df.to_csv(path, index=False)
        path = "C:\REPORTS/" + curr_reports[0][1] + "_LONDON.html"

        df = df[["CURRENCY", "OPEN_POSITION", "ENTRY_QTY", "ENTRY_EXE", "BUY_TP1", "BUY_TP2", "SELL_TP1", "SELL_TP2",
                 "BUY_EXIT_QTY", "BUY_EXIT_EXE", "SELL_EXIT_QTY", "SELL_EXIT_EXE"]]

        from IPython.display import HTML

        def hover(hover_color="E3FF33"):
            return dict(selector="tr:hover",
                        props=[("background-color", "%s" % hover_color)])

        styles = [
            hover(),
            dict(selector="th", props=[("font-size", "100%"), ("background-color", "FFCA33"),
                                       ("text-align", "center")]),
        ...
        html = df.style \
            .set_table_attributes('border="1" class=table table-stripped ') \
            .set_precision(10) \
            .set_table_styles([{'selector': '.row_heading, .blank', 'props': [('display', 'none;')]}]).set_table_styles(
            styles) \
            .set_properties(  # left-align the non-numeric columns and set their width
...
def ldn_HighLowfun():
    ...
    df_common_inputs = pd.read_excel('C:/database/FX_DAILY/FOREX_settings.xlsx', 'common_inputs')
    ldn_closetime = df_common_inputs["CLOSE TIME"].tail(1).iloc[-1].strftime("%H:%M:%S")
    lookbacktime = (datetime.strptime(ldn_closetime, "%H:%M:%S") + timedelta(minutes=30)).strftime("%H:%M:%S")
    ...
    for curr in london_list:
        ...
        asset_hist_data, histId = historicalHL(lookbacktime, contract, "5 D", histId + 1)

        asset_hist_data = asset_hist_data[asset_hist_data.open != -1]
        asset_hist_data = asset_hist_data.drop_duplicates(keep="last")

        asset_hist_data.index = asset_hist_data['date']
        if week_day != 0:
            asset_hist_data = asset_hist_data.loc[
                              (datetime.now() - timedelta(days=1)).strftime("%Y%m%d  ") + ldn_closetime:, ]
        else:
            # print("currency", curr)
            asset_hist_data = asset_hist_data.loc[
    ...
    time.sleep(120)
    close_list=[AUDNZD_report[5], CHFNOK_report[5], CHFSEK_report[5], NZDCHF_report[5], GBPSEK_report[5], EURNZD_report[5],
                        EURNOK_report[5], EURSEK_report[5], GBPNOK_report[5], NOKSEK_report[5]]
    ...
    path = "C:\REPORTS/" + datestring + "_LONDON.csv"
    ...
    for i in range(len(london_list)):
        ...
        csv_df.loc[csv_df.CURRENCY == london_list[i], 'EOD_CLOSE'] = close_list[i]
...
def ldn_HighLow():
    Thread(target=ldn_HighLowfun).start()
    return
...
def run_london():
    ...
    if shortsum > 1 and longsum > 1:
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, AUDNZD_params, ids, 100, 200, london_exit_df)).start()
        ################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, CHFNOK_params, ids1, 300, 400, london_exit_df)).start()
        ###############################################
        time.sleep(1)
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, CHFSEK_params, ids2, 500, 600,london_exit_df)).start()
        ##########################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'NZD.CHF', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, NZDCHF_params, ids3, 700, 800, london_exit_df)).start()
        time.sleep(1)
        ############################################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, GBPSEK_params, ids4, 900, 1000,london_exit_df)).start()
        ###############################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, EURNZD_params, ids5, 1100, 1200,london_exit_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, EURNOK_params, ids6, 1300, 1400,london_exit_df)).start()
        ##############################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, EURSEK_params, ids7, 1500, 1600, london_exit_df)).start()
        ##################################################################
        time.sleep(1)
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, GBPNOK_params, ids8, 1700, 1800,  london_exit_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'NOK.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(dat, NOKSEK_params, ids9, 1900, 1999, london_exit_df)).start()

        Thread(target=ldn_report).start()
        ########################
    return
...
if df_schedule.loc[df_schedule.DAY == "MONDAY", "PERMISSION_LONDON"].iloc[-1] == "YES" or \
    ...
    schedule.every().monday.at("12:59").do(ldn_HighLow)
if df_schedule.loc[df_schedule.DAY == "TUESDAY", "PERMISSION_LONDON"].iloc[-1] == "YES" or \
    ...
    schedule.every().tuesday.at("12:59").do(ldn_HighLow)
if df_schedule.loc[df_schedule.DAY == "WEDNESDAY", "PERMISSION_LONDON"].iloc[-1] == "YES" or \
    ...
    schedule.every().wednesday.at("12:59").do(ldn_HighLow)
...
if df_schedule.loc[df_schedule.DAY == "THURSDAY", "PERMISSION_LONDON"].iloc[-1] == "YES" or \
    ...
    schedule.every().thursday.at("12:59").do(ldn_HighLow)
...
if df_schedule.loc[df_schedule.DAY == "FRIDAY", "PERMISSION_LONDON"].iloc[-1] == "YES" or \
    ...
    schedule.every().friday.at("12:59").do(ldn_HighLow)
