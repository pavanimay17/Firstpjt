...
def LONDON(dat,params, ids, tickerId, histId, london_df):
    ...
    if london_list.index(params[0]) <= 4:
        ...
        if entry_result[0] > 0:
            ...
            tp_buy2 = round(float(((asset_hist_data['close'] - (tp1 * abs((pd.Series.rolling(
                (asset_hist_data['low'].shift(-1) / asset_hist_data['close'] - 1).shift(1),
                window=lookback).mean()).dropna()) * asset_hist_data['close'])).dropna()).iloc[-1]),
                            round_tp)
    ...
    print("ENTRY AND TP PARAMS: ",log_data)
    print("%%%%%%%%%%%%%%%%%%%%%%%%")
    print("")
        ...
        while pos != 0:
            ...
            if london_list.index(params[0]) <= 4:
                ...
                if condition_buy == "NOT FULFILLED":
                    ...
                    if rt_price[2] <= tp_buy and rt_price[0] <= tp_buy2:
                ...
                if condition_sell == "NOT FULFILLED":
                    ...
                    if rt_price[2] >= tp_sell and rt_price[1] >= tp_sell2:
    ...
    london_df.loc[london_df.CURRENCY == params[0], "CYCLE_DATE"] = list1[1][10]
...
def NY(dat,params, ids, tickerId, histId, ny_df):
    ...
    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
        ...
        while pos != 0:
            ...
            if ny_list.index(params[0]) <= 3:
                ...
                elif rt_price[2] >= tp_sell and rt_price[1] >= tp_sell2:
                    if condition_sell == "NOT FULFILLED":
                        signal = "SELL"
                        fetchtime = datetime.now().strftime("%Y%m%d %H:%M:%S")
                        print("CONDITION MET FOR", params[0], signal, "at", fetchtime,"MID PRICE:",rt_price[2], "TRYING TO PLACE AT:",rt_price[1])
    ...
    ny_df.loc[ny_df.CURRENCY == params[0], "CCYLE_DATE"] = list1[1][10]
...
def run_ny():
    ...
    if currtime < "00:26:00":
        sleeptime = (datetime.strptime("00:26:01", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
    ...
    if shortsum > 1 and longsum > 1:
        ...
        currtime = datetime.now().strftime('%H:%M:%S')
        if currtime < "00:29:20":
            sleeptime = (datetime.strptime("00:29:21", '%H:%M:%S') - datetime.strptime(currtime,
                                                                                       '%H:%M:%S')).total_seconds()
...
#         df_net.to_csv("C:/database\FX_DAILY/FOREX_NET.csv")
#         time.sleep(10)
